...
Asena.addCommand({ pattern: 'getjids ?(.*)', fromMe: true, desc: 'Get Jids name.' }, (async (message, match) => {
    if (match === '') return await message.sendMessage('Choose group or personal');
    let msg = '';
    let chats = message.client.chats.all();
    if (match == 'group') {
        for (let chat of chats) {
            if (chat.jid.endsWith('@g.us')) msg += `${chat.jid} : ${await getName(chat.jid, message.client)}\n`;
        }
    } else if (match == 'personal') {
        for (let chat of chats) {
            if (!chat.jid.endsWith('@g.us') && !chat.jid.endsWith('@broadcast')) msg += `${chat.jid} : ${await getName(chat.jid, message.client)}\n`;
        }
    }
...
Asena.addCommand({ pattern: 'block ?(.*)', fromMe: true, desc: Lang.BLOCK_DESC }, (async (message, match) => {
    if (message.reply_message !== false) {
        await message.sendMessage('@' + message.reply_message.jid.split('@')[0] + '```, ' + Lang.BLOCKED + '!```', {
            quotedMessage: message.reply_message.data, contextInfo: { mentionedJid: [message.reply_message.jid.replace('c.us', 's.whatsapp.net')] }
        });
        await message.client.blockUser(message.reply_message.jid, "add");
    } else if (message.mention !== false) {
        message.mention.map(async user => {
            await message.sendMessage('@' + user.split('@')[0] + '```, ' + Lang.BLOCKED + '!```', {
...
Asena.addCommand({ pattern: 'unblock ?(.*)', fromMe: true, desc: Lang.UNBLOCK_DESC }, (async (message, match) => {
    if (message.reply_message !== false) {
        ...
        await message.sendMessage('@' + message.reply_message.jid.split('@')[0] + '```, ' + Lang.UNBLOCKED + '!```', {
            quotedMessage: message.reply_message.data, contextInfo: { mentionedJid: [message.reply_message.jid.replace('c.us', 's.whatsapp.net')] }
        });
    } else if (message.mention !== false) {
        message.mention.map(async user => {
            await message.client.blockUser(user, "remove");
            await message.sendMessage('@' + user.split('@')[0] + '```, ' + Lang.UNBLOCKED + '!```', {
...
Asena.addCommand({ pattern: 'jid ?(.*)', fromMe: true, desc: Lang.JID_DESC }, (async (message, match) => {
    if (message.reply_message !== false) {
        ...
        message.mention.map(async user => { await message.sendMessage(user) });
    } else {
...
Asena.addCommand({ pattern: 'setstatus ?(.*)', fromMe: true, desc: 'set status.' }, (async (message, match) => {
    if (!message.reply_message) return message.sendMessage('*Reply to a message!*');
    else if (message.reply_message.image) {
        let media = await message.reply_message.downloadMediaMessage();
        return await message.setStatus(media, MessageType.image, { caption: match });
    } else if (message.reply_message.video) {
        let media = await message.reply_message.downloadMediaMessage();
        return await message.setStatus(media, MessageType.video, { caption: match });
...
Asena.addCommand({ pattern: 'setabout ?(.*)', fromMe: true, desc: 'set about.' }, (async (message, match) => {
