...
Asena.addCommand({ pattern: 'txt ?(.*)', fromMe: true, desc: Lang.OCR_DESC, usage: 'txt en' }, (async (message, match) => {
    if (message.reply_message === false || !message.reply_message.image) return await message.sendMessage(Lang.NEED_REPLY);
    await message.reply(Lang.DOWNLOADING);
    var location = await message.reply_message.downloadAndSaveMediaMessage('yxy');
    var dil;
    if (match !== '') {
