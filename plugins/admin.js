...
Asena.addCommand({ pattern: 'kick ?(.*)', fromMe: true, onlyGroup: true, desc: Lang.BAN_DESC }, (async (message, match) => {
    ...
    let user = await checkImAdmin(participants, !message.reply_message ? message.mention[0] : message.reply_message.jid)
    if (user) return await message.sendMessage("*User is admin*");
    if (message.reply_message != false) {
        await message.sendMessage('```' + `@${message.reply_message.jid.split('@')[0]} ${Lang.BANNED}` + '```', { contextInfo: { mentionedJid: [message.reply_message.jid] } });
        return await message.groupRemove(message.jid, message.reply_message.jid);
    } else if (message.reply_message === false && message.mention !== false) {
        await message.sendMessage('```' + `${message.mention[0].split('@')[0]} ${Lang.BANNED}` + '```', { contextInfo: { mentionedJid: message.mention } });
...
Asena.addCommand({ pattern: 'add(?: |$)(.*)', fromMe: true, onlyGroup: true, desc: Lang.ADD_DESC, usage: '.add 905xxxxxxxxx' }, (async (message, match) => {
    ...
    if (match !== '') {
        match.split(' ').map(async (user) => {
            ...
            return await message.sendMessage('```' + `${user} ${Lang.ADDED}` + '```');
        });
    } else {
        return await message.sendMessage(Lang.GIVE_ME_USER);
    }
...
Asena.addCommand({ pattern: 'promote ?(.*)', fromMe: true, onlyGroup: true, desc: Lang.PROMOTE_DESC }, (async (message, match) => {
    ...
    if (!im) return await message.sendMessage(Lang.IM_NOT_ADMIN);
    if (message.reply_message !== false) {
        let checkAlready = await checkImAdmin(participants, message.reply_message.jid);
        if (checkAlready) return await message.sendMessage(Lang.ALREADY_PROMOTED);
        await message.client.groupMakeAdmin(message.jid, [message.reply_message.jid]);
        return await message.sendMessage('@' + message.reply_message.jid.split('@')[0] + Lang.PROMOTED, { contextInfo: { mentionedJid: [message.reply_message.jid] } });
    } else if (message.reply_message === false && message.mention !== false) {
        let etiketler = '';
        ...
        return await message.sendMessage(etiketler + Lang.PROMOTED, { contextInfo: { mentionedJid: message.mention } });
    } else {
        return await message.sendMessage(Lang.GIVE_ME_USER);
    }
}));
...
Asena.addCommand({ pattern: 'demote ?(.*)', fromMe: true, onlyGroup: true, desc: Lang.DEMOTE_DESC }, (async (message, match) => {
    ...
    if (message.reply_message !== false) {
        ...
        await message.sendMessage('@' + message.reply_message.jid.split('@')[0] + Lang.DEMOTED, { contextInfo: { mentionedJid: [message.reply_message.jid] } });
        return await message.client.groupDemoteAdmin(message.jid, [message.reply_message.jid]);
    } else if (message.reply_message === false && message.mention !== false) {
        let etiketler = '';
        message.mention.map(async (user) => {
            let checkAlready = await checkImAdmin(participants, user);
        ...
        await message.sendMessage(etiketler + Lang.DEMOTED, { contextInfo: { mentionedJid: message.mention } });
...
Asena.addCommand({ pattern: 'common ?(.*)', fromMe: true, desc: 'Shows common members in groups.' }, (async (message, match) => {
    let [jid1, jid2] = match.split(' ') || []
    if (jid1 == '' || jid2 == '' || jid1 == undefined || jid2 == undefined) return await message.sendMessage('*Syntax Error!*', { quoted: message.data });
    try {
        var grup1 = await message.groupMetadata(jid1);
        var grup2 = await message.groupMetadata(jid2);
    } catch (error) {
        return await message.sendMessage("*I don't have permission to acess these groups!*", { quoted: message.data });
    }
    let common = '';
    ...
    return await message.sendMessage(common, { contextInfo: { mentionedJid: com } });
}));

Asena.addCommand({ pattern: 'diff ?(.*)', fromMe: true, desc: 'Shows members not in two groups.' }, (async (message, match) => {
    let [jid1, jid2] = match.split(' ') || []
    if (jid1 == '' || jid2 == '' || jid1 == undefined || jid2 == undefined) return await message.sendMessage('*Syntax Error!*', { quoted: message.data });
    try {
        var grup1 = await message.groupMetadata(jid1);
    ...
    return await message.sendMessage(diff, { contextInfo: { mentionedJid: difference } });
}));

Asena.addCommand({ pattern: 'join ?(.*)', fromMe: true, desc: 'Join Groups.' }, (async (message, match) => {
    match = match === '' ? message.reply_message.text : match
    if (match == '') return await message.sendMessage("*GIVE ME A WA INVITE LINK*");
