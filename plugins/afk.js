...
Asena.addCommand({ on: 'text', fromMe: false, deleteCommand: false }, (async (message, match) => {
    if (global.AFK.isAfk && !message.fromMe && message.isGroup && ((message.mention !== false && message.mention.length !== 0) || message.reply_message !== false)) {
        if (message.isGroup && message.mention.length !== 0 && message.reply_message === false) {
            message.mention.map(async (jid) => {
                if (message.client.user.jid.split('@')[0] === jid.split('@')[0]) {
                    return await message.sendMessage(Lang.AFK_TEXT + '\n' +
                        (global.AFK.reason !== false ? '\n*' + Lang.REASON + ':* ```' + global.AFK.reason + '```' : '') +
                        (global.AFK.lastseen !== 0 ? '\n*' + Lang.LAST_SEEN + '* ```' + secondsToHms(Math.round((new Date()).getTime() / 1000) - global.AFK.lastseen) + ' before.```' : ''), { quoted: message.data });
                }
            })
...
Asena.addCommand({ pattern: 'afk ?(.*)', fromMe: true, deleteCommand: false, desc: Lang.AFK_DESC }, (async (message, match) => {
    if (!global.AFK.isAfk) {
        ...
        return await message.sendMessage(Lang.IM_AFK + (global.AFK.reason !== false ? ('\n*' + Lang.REASON + ':* ```' + global.AFK.reason + '```') : ''));
    }
}));
