...
let fm = true;
Asena.addCommand({ pattern: 'filter ?(.*)', fromMe: fm, desc: Lang.FILTER_DESC }, (async (message, match) => {
    match = message.message.match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
...
Asena.addCommand({ pattern: 'stop ?(.*)', fromMe: fm, desc: Lang.STOP_DESC }, (async (message, match) => {
    match = message.message.match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
    ...
    filtreler.map(
        async (filter) => {
            ...
            if (pattern.test(message.message)) {
                await message.sendMessage(filter.dataValues.text, { quoted: message.data });
            }
        }
    );
}));