...
Asena.addCommand({ pattern: 'update now$', fromMe: true, desc: "To-Up-Date" }, (async (message, match) => {
    let isupdate = await updateChecker()
    if (!isupdate) return await message.sendMessage('*Bot is up-todate.*\n*Nothing to Update.')
    if (Config.HEROKU.HEROKU) {
        await message.reply("```Updating...```");
        try {
            var app = await heroku.get('/apps/' + Config.HEROKU.APP_NAME)
        } catch {
            await message.sendMessage("*Invalid Heroku Details*\n*Update Heroku vars*");
        }
        git.fetch('upstream', Config.BRANCH);
        git.reset('hard', ['FETCH_HEAD']);
        let git_url = app.git_url.replace("https://", "https://api:" + Config.HEROKU.API_KEY + "@")
        try {
